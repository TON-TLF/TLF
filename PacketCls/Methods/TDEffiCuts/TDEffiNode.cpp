 ...
 */
void TDEffiNode::Create(ProgramState *ps, int depth) {
    /** 对规则数量进行必要的检查 */
    if (rules_num == 0){
        cout << ">>> TDEffiNode::Create -> Wrong! rules_num = 0!" << endl;
        exit(1);
    }
    HasBounds();

    /** 如果启用了冗余移除且节点深度不为0，移除冗余规则 */ 
    if (TDEffiCuts::remove_redund && depth != 0) {
        RemoveRedund();
    }
	...
	/** 计算节点的流行度*/ 
	double pop = TDEffiCuts::total_trace_num;
    double traces_sum = 0;
    if(TDEffiCuts::total_trace_num != 0){
        for(int i = 0; i < 2; i++){
    ...
    int _max_depth = TDEffiCuts::max_depth, _leaf_size = TDEffiCuts::leaf_size;
    if(TDEffiCuts::total_trace_num != 0){
        if      (pop > TDEffiCuts::pop_add_2) _max_depth += 2;
        else if (pop > TDEffiCuts::pop_add_1) _max_depth += 1;
        else if (pop < TDEffiCuts::pop_dec_1) _max_depth -= 1;
        else if (pop < TDEffiCuts::pop_dec_2) _max_depth -= 2;
        _leaf_size /= max(min(pop, TDEffiCuts::pop_max), TDEffiCuts::pop_min);
    }
    ...
    child_arr = new TDEffiNode*[child_num];
    ...
    for (int i = 0; i < cuts[0]; ++i){
        for (int j = 0; j < cuts[1]; ++j) {
            ...
            child_arr[index] = new TDEffiNode(); 
            vis[index] = false; 
            ...
            for (int k = 0; k < 5; ++k) {
                ...
                if (k == dims[0] || k == dims[1]) {
                    ...
                    /** 如果低位大于高位，释放子节点并退出循环 */ 
                    if (low > high) {
                        cout<<">>> TDEffiNode::Create -> Wrong! low > high error!"<<endl;
                        exit(1);
                        break;
                    }
    ...
    for (int i = 0; i < child_num; ++i){
        if(vis[i] == true && child_arr[i] == NULL){
            cout<<">>> TDEffiNode::Create -> Wrong! child_arr error!"<<endl;
            exit(1);
        }
    }

    /** 如果启用了节点合并，尝试合并相同规则的子节点 */ 
    if (TDEffiCuts::node_merge) {
        for (int i = 0; i < child_num; ++i){
            if (vis[i] == true){
                for (int j = 0; j < i; ++j) {
 ...
 * @return 内存大小（字节）。
 */
uint64_t TDEffiNode::CalMemory() {
	uint64_t size = sizeof(TDEffiNode);
	size += rules.capacity() * sizeof(Rule*);
    if (child_arr != nullptr){
        size += child_num * sizeof(TDEffiNode*);
        map<TDEffiNode*, int> child_map;
        for (int i = 0; i < child_num; ++i){
 ...
 */
void TDEffiNode::HasBounds() {
    for(int i = 0; i < 5; ++i){
        if(bounds[i][0] > bounds[i][1]){
            cout<<"TDEffiNode::Create -> Wrong! bounds error!"<<endl;
            exit(1);
        }
    }
}
 ...
 * @brief 计算节点的切割维度
 */
void TDEffiNode::CalDimToCuts(int traces_num) {
    if (rules_num == 0){
        cout << ">>> TDEffiNode::CalDimToCuts -> Wrong! rules_num = 0" << endl;
        exit(1);
    }
	...
	for(int d = 0; d < 5; ++d) {
        ...
        if(uni_sum == 0 || uniformity[d] == -1){
            ...
            score[d] += TDEffiCuts::wfac * (diff_range[d] / diff_sum);
            score[d] += (1 - TDEffiCuts::wfac) * (uniformity[d] / uni_sum );
        }
        score_sum += score[d];
	}
    ...
    if(index1 == -1){
        cout<<">>> TDEffiNode::CalDimToCuts -> Wrong! index1 error!"<<endl;
        exit(1);
    }

    if(index2 == -1){
        cout<<">>> TDEffiNode::CalDimToCuts -> Wrong! index2 error!"<<endl;
        exit(1);
    }
}
 ...
 * @param rules 规则指针的引用
 */
void TDEffiNode::CalNumCuts(double pop) {
    if (rules_num == 0){
        cout << ">>> TDEffiNode::CalNumCuts -> Wrong! rules_num = 0" << endl;
        exit(1);
    }

    /** 根据树的类型和节点的占用率调整切割因子 */ 
    double _spfac = TDEffiCuts::spfac;
    if(TDEffiCuts::total_trace_num != 0){
        if      (pop > TDEffiCuts::pop_mul_2)     _spfac *= 1.2;
        else if (pop > TDEffiCuts::pop_mul_1)   _spfac *= 1.1;
        else if (pop < TDEffiCuts::pop_div_1)   _spfac /= 1.1;
        else if (pop < TDEffiCuts::pop_div_2)   _spfac /= 1.2;
        // else if (pop < 0.001) _spfac /= 60.0;
    }
    ...
    if(cuts[0] <= 0 || cuts[0] > (uint64_t)bounds[dims[0]][1] - bounds[dims[0]][0] + 1){
        ...
        cout<<">>> TDEffiNode::CalNumCuts -> Wrong! cuts[0] error!"<<endl;
        exit(1);
    }
    if(cuts[1] <= 0 || cuts[1] > (uint64_t)bounds[dims[1]][1] - bounds[dims[1]][0] + 1){
        cout<<">>> TDEffiNode::CalNumCuts -> Wrong! cuts[1] error!"<<endl;
        exit(1);
    }
    ...
    if(width[0] == 0 || width[1] == 0){
        cout<<">>> TDEffiNode::CalNumCuts -> Wrong! width error!"<<endl;
        exit(1);
    }
}
 ...
 * @return bool 返回判断结果
 */
bool TDEffiNode::NodeContainRule(Rule* rule) {
	for (int i = 0; i < 5; i++)
...
bool SameEffiRules(TDEffiNode *node1, TDEffiNode *node2) {
    if(node1 == NULL || node2 == NULL){
        cout<<">>> TDEffiNode::SameEffiRules -> Wrong! NULL error!"<<endl;
        exit(1);
    }
