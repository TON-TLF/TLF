 ...
 */
void HcNode::Create(ProgramState *ps, int depth) {
    /** 对规则数量进行必要的检查 */
    if (rules_num == 0){
        cout << ">>> HcNode::Create -> Wrong! rules_num = 0!" << endl;
        exit(1);
    }
    HasBounds();

    /** 如果启用了冗余移除且节点深度不为0，移除冗余规则 */ 
    if (HyperCuts::remove_redund && depth != 0) {
        RemoveRedund();
    }
	sort(rules.begin(), rules.end(), CmpRulePriority);

    /** 检查是否达到最大深度、规则数量小于等于叶节点阈值或节点无边界 */ 
    if (depth == HyperCuts::max_depth || rules_num <= HyperCuts::leaf_size) {
        leaf_node = true;      
    ...
    child_arr = new HcNode*[child_num];
        ...
        for (int j = 0; j < cuts[1]; ++j) {
            ...
            for (int k = 0; k < 5; ++k) {
                ...
                if (k == dims[0] || k == dims[1]) {
                    ...
                    /** 如果低位大于高位，释放子节点并退出循环 */ 
                    if (low > high) {
                        cout<<">>> HcNode::Create -> Wrong! low > high error!"<<endl;
                        exit(1);
                        break;
                    }
    ...
    for (int i = 0; i < child_num; ++i){
        if(vis[i] == true && child_arr[i] == NULL){
            cout<<">>> HcNode::Create -> Wrong! child_arr error!"<<endl;
            exit(1);
        }
    }

    /** 如果启用了节点合并，尝试合并相同规则的子节点 */ 
    if (HyperCuts::node_merge) {
        for (int i = 0; i < child_num; ++i){
            if (vis[i] == true){
                for (int j = 0; j < i; ++j) {
 ...
 * @return 内存大小（字节）。
 */
uint64_t HcNode::CalMemory() {
    uint64_t size = sizeof(HcNode);
    size += rules.capacity() * sizeof(Rule*);
    if (child_arr != nullptr) {
        size += child_num * sizeof(HcNode*);
        map<HcNode*, int> child_map;
        for (int i = 0; i < child_num; ++i) {
 ...
 */
void HcNode::HasBounds() {
    for(int i = 0; i < 5; ++i){
        if(bounds[i][0] > bounds[i][1]){
            cout<<">>> HcNode::HasBounds -> Wrong! bounds error!"<<endl;
            exit(1);
        }
    }
}
 ...
 * @brief 计算节点的切割维度
 */
void HcNode::CalDimToCuts() {
    if (rules_num == 0){
        cout << ">>> HcNode::CalDimToCuts -> Wrong! rules_num = 0" << endl;
        exit(1);
    }
    ...
    if(index1 == -1){
        cout<<">>> HcNode::CalDimToCuts -> Wrong! index1 error!"<<endl;
        exit(1);
    }

    if(index2 == -1){
        cout<<">>> HcNode::CalDimToCuts -> Wrong! index2 error!"<<endl;
        exit(1);
    }
}
 ...
 * @param rules 规则指针的引用
 */
void HcNode::CalNumCuts() {
    if (rules_num == 0){
        cout << ">>> HcNode::CalNumCuts -> Wrong! rules_num = 0" << endl;
        exit(1);
    }

    uint64_t spmf = rules_num * HyperCuts::spfac;
    ...
    if(cuts[0] <= 0 || cuts[0] > (uint64_t)bounds[dims[0]][1] - bounds[dims[0]][0] + 1){
        ...
        cout<<">>> HcNode::CalNumCuts -> Wrong! cuts[0] error!"<<endl;
        exit(1);
    }
    if(cuts[1] <= 0 || cuts[1] > (uint64_t)bounds[dims[1]][1] - bounds[dims[1]][0] + 1){
        cout<<">>> HcNode::CalNumCuts -> Wrong! cuts[1] error!"<<endl;
        exit(1);
    }
    ...
    if(width[0] == 0 || width[1] == 0){
        cout<<">>> HcNode::CalNumCuts -> Wrong! width error!"<<endl;
        exit(1);
    }
}
 ...
 * @return bool 返回判断结果
 */
bool HcNode::NodeContainRule(Rule* rule) {
	for (int i = 0; i < 5; i++)
...
bool SameEffiRules(HcNode *node1, HcNode *node2) {
    if(node1 == NULL || node2 == NULL){
        cout<<">>> HcNode::SameEffiRules -> Wrong! NULL error!"<<endl;
        exit(1);
    }
